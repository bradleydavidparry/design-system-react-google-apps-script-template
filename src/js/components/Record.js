...
import { formatDate, normalise } from "../functions/utilities";
...
const getCurrentRecord = (data,recordId,query,schema,splitFields) => {
    if(recordId !== "add_new") return data.filter(row => row.ID == recordId)[0];
...
export default function Record (props){
    ...
    const { userType, dataObject, setDataObject, user, lookups }  = useContext(AppContext);
    ...
    const updateFormData = (newFormData) => {
        newFormData = processFormData(schema,newFormData,lookups,viewName)
        setFormData(newFormData);
    }
    

    function parseRevealCondition(revealConditionString){
        return revealConditionString.split("||").reduce((object,conditionString) => {
    ...
    function isRevealConditionMet(revealConditionString) {
        ...
        return Object.keys(revealConditionObject).some(key => revealConditionObject[key].includes(formData[normalise(key)]))
    }
    ...
    return (
        <div className="govuk-grid-row">
            <div className={fullWidthView ? "govuk-grid-column-two-thirds" : "govuk-grid-column-full" }>
                <Fieldset>
                    ...
                    {Object.keys(errors).length > 0 ? 
                        ...
                        /> : null}
                    {canEditView && showSubmitButton ? <Button onClick={() => submitFunction(
                        {setSubmitting,
                        splitFields,
                        normalise,
                        schema,
                ...
                <ViewComponent Name={viewName} data={formData} schema={schema} type={'below_record'}  />
            </div>
        </div>
    )
}
